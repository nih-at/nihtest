...
class Test:
    def __init__(self, configuration, args):
        ...
        if file_name[-5:] != ".test":
            file_name += ".test"
        self.file_name = self.find_file(file_name)
    ...
    def run(self):
        print(f"Running test {self.args.testcase}")

    def find_file(self, filename):
        # TODO: search in source directories
        return filename
    ...
    def parse_case(self):
        ...
        for name, directive in Test.directives.items():
    ...
    def parse_line(self, line):
        ...
        if name not in Test.directives:
            self.error(f"unknown directive '{name}'")
            return

        directive = Test.directives[name]
